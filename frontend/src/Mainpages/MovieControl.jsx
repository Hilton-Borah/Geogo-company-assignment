...
const MovieControl = () => {
  ...
  console.log(searchparams.getAll("page")[0])
  ...
  return (
    <div className='mt-36 w-11/12 border lg:flex justify-between m-auto pt-10 pb-10'>
      <form onSubmit={handleSubmit} className='border-r lg:w-2/5 md:full lg:shrink-0'>
        ...
        <div className='lg:w-4/5 md:flex md:justify-between md:items-center border gap-5 mx-auto'>
          <input className='w-4/5 md:w-2/5 lg:w-1/2 rounded-md outline-0 mb-5 md:mb-5' placeholder='Enter rating*' value={imdbRating} name='imdbRating' type="number" onChange={handleChange} required /><br />
          <input className='w-4/5 md:w-2/5 lg:w-1/2 rounded-md outline-0 mb-5 md:mb-5' placeholder='Enter duration*' value={Runtime} name='Runtime' onChange={handleChange} type="text" required /><br />
        </div>

        <div className='lg:w-4/5 lg:flex justify-between items-center gap-5 m-auto'>
          <input className='sm:w-4/5 rounded-md outline-0 mb-5' placeholder='Enter country*' value={Country} name='Country' type="text" onChange={handleChange} required /><br />
          <input className='sm:w-4/5 rounded-md outline-0 mb-5' placeholder='Enter release year' value={Year} name='Year' onChange={handleChange} type="number" required /><br />
        </div>

        <div className='lg:w-4/5 lg:flex justify-between items-center gap-5 m-auto'>
          {/* <div className='flex justify-between items-center gap-5 m-auto'> */}
          <input className='sm:w-4/5 rounded-md outline-0 mb-5' placeholder='Enter released date*' value={Released} onChange={handleChange} name='Released' type="text" required /><br />
          {/* </div> */}
          <input className='sm:w-4/5 rounded-md outline-0 mb-5' placeholder='Enter language*' value={Language} name='Language' type="text" onChange={handleChange} required /><br />
        </div>
        ...
        <div className='w-11/12 m-auto'>
          {
            allMovie && allMovie.map((el) => {
              return (
                <div key={el.id} className='flex justfy-between items-center mt-2 pb-1 cursor-pointer border-b'>
                  ...
                  <Link to={`/${el.Title}/details/${el._id}`}><img className='w-5 h-5' src={el.Poster} alt="" /></Link>
                    <Link to={`/${el.Title}/details/${el._id}`}><p className='lg:text-sm md:text-xs'>{el.Title}</p></Link>
                    <p className='md:block'>{el.Year}</p>
                  </div>
                  <div className='w-1/5 flex justfy-between items-center gap-5'>

                    <button className='flex justfy-between items-center gap-2 rounded text-blue hover:bg-blue hover:text-white border-blue border-2 pl-2 pr-2 text-xs'><AiOutlineEdit />Edit</button>

                    <button className='flex justfy-between items-center gap-2 rounded text-red hover:bg-red hover:text-white border-red border-2 pl-2 pr-2 text-xs'><AiOutlineDelete />Delete</button>
                  </div>
                </div>
